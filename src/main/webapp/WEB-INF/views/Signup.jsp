...
/* login link */
.login-link{
  text-align:center;font-size:13px;color:var(--sub);margin-top:22px;
}
...
.success-sub{font-size:13.5px;color:var(--sub);margin-bottom:24px}
</style>
</head>
<body>
  ...
  <div class="lp-inner">
    ...
    <div class="lp-stats">
      <div class="lp-stat">
        ...
        <div class="lp-stat-lbl">Events Hosted</div>
      </div>
      <div class="lp-stat">
        <div class="lp-stat-num">2K+</div>
        <div class="lp-stat-lbl">Participants</div>
      </div>
      <div class="lp-stat">
        <div class="lp-stat-num">₹5L+</div>
        <div class="lp-stat-lbl">Prize Pool</div>
      </div>
      <div class="lp-stat">
        <div class="lp-stat-num">30+</div>
        <div class="lp-stat-lbl">Organisations</div>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="form-box">
    ...
    <div class="steps" id="stepsBar">
      ...
      <div class="step-wrap">
        ...
        <div class="step-lbl">Contact</div>
      </div>
    ...
    <form action="usersignup" method="post" id="signupForm" novalidate>
    ...
    <!-- Success box (shown after submit if needed) -->
    <div class="success-box" id="successBox">
      <div class="success-icon">✓</div>
      <div class="success-title">Account Created! 🎉</div>
      <div class="success-sub">Welcome to CodeNova SOU. Redirecting to login...</div>
      <a href="login" class="btn-pri" style="text-decoration:none;display:inline-flex;width:auto;padding:11px 28px">Go to Login</a>
    </div>
...
function goStep(n) {
  ...
  // update dots
  updateDots(currentStep, n);
  currentStep = n;
  document.getElementById('step' + currentStep).classList.add('active');
}

function updateDots(from, to) {
  for (var i = 1; i <= 3; i++) {
    var dot  = document.getElementById('dot' + i);
    dot.classList.remove('active','done');
    if (i < to)       dot.classList.add('done');
    else if (i === to) dot.classList.add('active');
  }
  for (var j = 1; j <= 2; j++) {
    var line = document.getElementById('line' + j);
    line.classList.toggle('done', j < to);
  }
}
...
function validateStep(step) {
  if (step === 1) {
    ...
    var errG = document.getElementById('err-gender');
    if (!genderChecked) {
      errG.classList.add('show'); ok = false;
    } else {
...
function validateStep3() {
  ...
  var errCpw = document.getElementById('err-confirm');
  if (pw.length < 8) {
    document.getElementById('password').classList.add('error');
    errPw.classList.add('show'); ok = false;
  } else {
    document.getElementById('password').classList.remove('error');
    errPw.classList.remove('show');
  }
  if (pw !== cpw || cpw.length === 0) {
    document.getElementById('confirmPassword').classList.add('error');
    errCpw.classList.add('show'); ok = false;
  } else {
    document.getElementById('confirmPassword').classList.remove('error');
    errCpw.classList.remove('show');
  }
  return ok;
}
...
function val(id, test) {
  ...
  if (!err) err = document.getElementById('err-' + (id === 'mobileNumber' ? 'mobile' : id));
...
function checkStrength(pw) {
  ...
  for (var i = 1; i <= 4; i++) {
    ...
    bar.style.background = i <= score ? colors[score - 1] : 'var(--line)';
  }
